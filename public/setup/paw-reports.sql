...
drop policy if exists "walk_points_all_own" on public.walk_track_points;
...
drop policy if exists "paw_reports_all_own" on public.paw_reports;
create policy "paw_reports_all_own" on public.paw_reports
  for all using (auth.uid() = owner_id) with check (auth.uid() = owner_id);

drop policy if exists "paw_media_all_own" on public.paw_report_media;
...
-- Open access while Anna is single-operator (safe to keep until you lock RLS later)
drop policy if exists "walks_dev_all" on public.walks;
create policy "walks_dev_all" on public.walks for all using (true) with check (true);
drop policy if exists "walk_points_dev_all" on public.walk_track_points;
create policy "walk_points_dev_all" on public.walk_track_points for all using (true) with check (true);
drop policy if exists "paw_reports_dev_all" on public.paw_reports;
create policy "paw_reports_dev_all" on public.paw_reports for all using (true) with check (true);
drop policy if exists "paw_media_dev_all" on public.paw_report_media;
...
drop policy if exists "paw_reports_public_read_sent" on public.paw_reports;
create policy "paw_reports_public_read_sent" on public.paw_reports
  for select using (status = 'sent');

drop policy if exists "paw_media_public_read_sent" on public.paw_report_media;
